const cMarsIntro = document.querySelector("#cMarsIntro");
const contextM = cMarsIntro.getContext("2d");
...
function resize() { 
    cMarsIntro.width = window.innerWidth;
    cMarsIntro.height = window.innerHeight;
    buildField();
    buildParticles();
...
function buildField() {
    ...
    columns = Math.ceil(cMarsIntro.width / size) + 1;
    rows = Math.ceil(cMarsIntro.height / size) + 1;
    field = [];
...
function buildParticles() {
    for (let i = 0; i < particlesToAdd; i++){
        particles.push({
            position: new Vector(
                cMarsIntro.width * Math.random(),
                cMarsIntro.height * Math.random()
            ),
            velocity: new Vector(
                Math.random() * 2 - 1,
                Math.random() * 2 - 1,
...
function updateParticles() {
    for (let i = 0; i < particles.length; i++){
        ...
        if (particle.position.x < 0) {
            particle.position.x = cMarsIntro.width - particle.size;
        } else if (particle.position.x > cMarsIntro.width) {
            particle.position.x = 0 + particle.size;
        }

        if (particle.position.y < 0) {
            particle.position.y = cMarsIntro.height - particle.size;
        } else if (particle.position.y > cMarsIntro.height) {
...
function drawParticles() {
    for (let i = 0; i < particles.length; i++){
        const particle = particles[i];
        contextM.beginPath();
        contextM.arc(particle.position.x, particle.position.y, particle.size, 0, TAU);
        contextM.fillStyle = "hsla("+particle.hue+", 0%, 0%, 0.2)";
        contextM.fill();
    }
}
